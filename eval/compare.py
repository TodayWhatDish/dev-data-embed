...
def load(model: str) -> dict:
    ...
    if not path.exists():
        raise SystemExit(
            f"결과 파일이 없습니다: {path}\nEMBED_MODEL={model} 로 재색인 후 eval.py 를 돌리세요."
        )
...
def compare(model_a: str, model_b: str, k: int = 3) -> None:
    ...
    if len(shared) != len(rank_a) or len(shared) != len(rank_b):
        print(
            f"경고: 표본이 다르다 (A {len(rank_a)}건 / B {len(rank_b)}건 / 공통 {len(shared)}건). "
    ...
    print(
        f"A = {model_a} ({a['dim']}차원, 질의 {a['metrics'].get('query_ms', 0):.1f}ms) mrr {a['metrics']['mrr']:.3f}"
    )
    print(
        f"B = {model_b} ({b['dim']}차원, 질의 {b['metrics'].get('query_ms', 0):.1f}ms) mrr {b['metrics']['mrr']:.3f}"
    )
    ...
    print(
        f"순위 이동: B가 올린 표본 {better}건 / 내린 표본 {worse}건 / 동일 {len(shared) - better - worse}건"
    )
