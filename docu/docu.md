...
스키마(`SCHEMA.md`)에는 담기지 않지만 지켜야 하는 것들. **DB 가 강제하지 못하므로 앱·운영이 책임진다.**
...
- [ ] `products.target_size` 는 `pets.size` 와 같은 `1~5` 코드를 쓴다.
...
`create_schema.py` 에 아직 없는 것들.

- [ ] `ingredients` — 원료 마스터. `allergen_id` 가 안전 판정의 연결 고리
- [ ] `products` / `product_ingredients` / `product_nutrition`
- [ ] `feeding_purposes` / `product_feeding_purposes`
- [ ] `purchases`
- [ ] `reviews` / `review_embeddings`
- [ ] 뷰 (`v_pet_context`, `v_review_docs`, `v_product_safety` 등)
...
- [ ] `src/make_db/` 제거
- [ ] `README.md` / `CLAUDE.md` / `DESIGN.md` 의 `src/make_db/create_db_schema.py` 경로 참조 정리
