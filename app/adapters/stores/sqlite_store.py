...
class SqliteVectorStore:
    ...
    def hashes(self, kind: str, *, ids=None) -> dict[str, str]:
        ...
        cur = self._con.cursor()
        try:
            rows = cur.execute(f"SELECT purchase_id, chunk_index, source_hash FROM {table}").fetchall()
    ...
    def delete(self, kind, ids) -> None:
        ...
        cur.executemany(f"DELETE FROM {table} WHERE purchase_id = ? AND chunk_index = ?", pairs)
        self._con.commit()
