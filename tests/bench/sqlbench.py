...
def _demo():
    ...
    compare_fn(
        {"문자열": lambda: "a,b", "리스트": lambda: ["b", "a"]},
        n=3,
        key=lambda v: sorted(v.split(",") if isinstance(v, str) else v),
    )
    try:
    ...
    assert throughput_fn({"터지는 것": boom}, threads=(1,), seconds=0.05)[0][2] > 0, (
        "throughput_fn 이 실패를 안 세고 있다"
    )
