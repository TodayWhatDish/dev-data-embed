...
BREED_COUNT_DIST = {1: [(0, 8), (1, 70), (2, 20), (3, 2)], 2: [(0, 4), (1, 86), (2, 10), (3, 0)]}
    ...
    for u in users:
        ...
        for _ in range(pick([(1, 70), (2, 24), (3, 6)])):
            ...
            if as_dt(birth) > ceiling:
                birth = rdate(
                    max(user_created.date(), ceiling.date() - timedelta(days=365 * 15)), ceiling.date()
                )
            ...
            pets.append(
                (
                    ...
                    pick([(1, 65), (0, 35)]),
                    dt(inactive) if inactive else None,
                    dt(created),
                    dt(updated),
                )
            )
            ...
            if rng.random() < 0.47:
                ...
                for _ in range(pick([(1, 80), (2, 20)])):
                    ...
                    picked.add(rng.choice(ALLERGEN_LEAVES if rng.random() < 0.75 else ALLERGEN_NODES))
...
def gen_products():
    ...
    for pid in range(1, N_PRODUCTS + 1):
        ...
        age_min, age_max = {"all": (0, 1200), "puppy": (0, 12), "senior": (84, 1200), "adult": (12, 1200)}[
            stage
        ]
        ...
        animal_txt = {(): "대상 축종 미등록", (1,): "개 전용", (2,): "고양이 전용", (1, 2): "개·고양이 공용"}[
            animals
        ]
        size_txt = (
            f"{SIZE_LABEL[size_min]} 전용"
            if size_min == size_max
            else "전 체구"
            if (size_min, size_max) == (1, 5)
            else f"{SIZE_LABEL[size_min]}~{SIZE_LABEL[size_max]}"
        )
        purpose_txt = (
            "/".join(PURPOSE_NAME[p] for p in sorted(chosen_purposes)) + " 관리"
            if chosen_purposes
            else "일상 급여"
        )
        description = (
            f"{animal_txt} {form} {label}. {size_txt}, {STAGE_LABEL[stage]} 대상. "
        ...
        if rng.random() < NUTRITION_RATE[cat]:
            ...
            nutrition.append(
                (pid, protein, fat, fiber, ash, moisture, maybe(0.6, 2.0), maybe(0.5, 1.6), maybe(0.1, 0.6))
            )
...
def review_body(pet, product, rating, age_month, allergen, purposes, ingredients):
    ...
    if ingredients and rng.random() < 0.5:
        ...
        parts.append(
            rng.choice([f"{picked} 들어간 게 마음에 들어서 골랐어요.", f"{picked} 배합이라길래 믿고 샀어요."])
        )
...
def from_source(pets, products, animals_of, allergen_of, purpose_of, ingredient_of):
    ...
    for src in read_csv(SOURCE_REVIEWS):
        ...
        if pet_id not in pet_by_id:
            ...
            pet_id = (
                min(same_user, key=lambda r: r[0])[0]
                if same_user
                else min(pets, key=lambda r: (abs(r[7] - want), r[0]))[0]
            )
            remapped += 1
        ...
        if target and pet[2] not in target:
            product = rng.choice(by_animal_cat.get((pet[2], product[1])) or by_animal[pet[2]])
            swapped += 1
        ...
        purchases.append(
            (purchase_id, pet_id, product[0], int(src["quantity"]), product[5], age_month, pet[7], dt(bought))
        )
        reviews.append(
            (
                purchase_id,
                ...
                review_body(
                    ...
                    rating,
                    age_month,
                    allergen_of.get(pet_id),
                    purpose_of.get(product[0]),
                    ingredient_of.get(product[0]),
                ),
                int(src["is_holdout"]),
                dt(bought),
            )
    ...
    print(
        f"  {SOURCE_REVIEWS.name} {len(purchases)}건 연결 (펫 보정 {remapped}건, 축종 안 맞아 제품 교체 {swapped}건)"
    )
...
def gen_purchases(pets, products, p_animals, pet_allergies, purposes, p_ings):
    ...
    for purchase_id in range(next_id, next_id + n):
        ...
        bought = rdt(max(datetime.fromisoformat(pet[11]), datetime.fromisoformat(product[15])), NOW)
...
def main():
    ...
    write_csv(
        ...
        [
            ...
            "target_size_min",
            "target_size_max",
            "target_age_min_month",
            "target_age_max_month",
            "description",
    ...
    print(
        f"제품 {N_PRODUCTS}개 중 verified=0 {sum(1 for p in products if p[13] == 0)}개 / "
