...
검색/추천 후보를 고르는 로직(candidates())은 services/searching.py로 옮겼다.
...
CLIENT_FAULT = {
    ...
    "constraint_check": (InvalidInput, "입력 값이 허용 범위를 벗어났습니다."),
    "constraint_fk": (InvalidInput, "참조하는 대상이 없습니다."),
    "constraint_notnull": (InvalidInput, "필수 값이 비었습니다."),
    "bad_range": (InvalidInput, "페이지 번호나 크기가 잘못되었습니다."),
}


def list_products(page: int, size: int) -> list[dict]:
    """상품 여러 건 조회. page/size 가 잘못되면 InvalidInput 이다.
    ...
    try:
        ...
        exc_cls, msg = fault
        logging.getLogger().info(f"List products -> {exc_cls.__name__}, page: {page}, size: {size}")
...
def get_product(product_id: int) -> dict:
    ...
    if not product:
        raise NotFound(f"product_id {product_id}상품이 없다.")
    return product
...
def update_product(product_id: int, values: dict) -> int:
    ...
    if not values:  # PATCH 빈 바디. 안 막으면 'SET  WHERE' 라는 깨진 SQL 이 나간다
        raise InvalidInput("조건 입력이 잘못되었습니다.")
    try:
    ...
    logging.getLogger().debug(
        f"Try Update product table, updated_row: {updated_row}, where: {product_id},  update_cols: {values.keys()}"
    )
...
def update_after_select_product(product_id: int, values: dict) -> tuple[int, dict]:
    ...
    if updated_row == 0:
        raise NotFound(f"{product_id}상품이 없다.")
...
def delete_product(product_id: int) -> None:
    ...
    없는 id 는 0행이라 NotFound 다 (선조회 대신 고친 행 수로 안다).
    """
    try:
    ...
    if updated_row == 0:
        raise NotFound(f"비활성화(삭제)할 {product_id}상품이 없다.")
