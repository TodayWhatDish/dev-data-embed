...
if __name__ == "__main__":
    ...
    run(
        f"A. 축종 이름 하나 (pet {typical_pet}) - 전부 1문장이라 조인/캐시만 갈린다",
        {
    ...
    run(
        f"B. 알레르겐 이름 {heavy_n}개 (pet {heavy_pet}) - 캐시 조회가 행 수만큼 늘어 제일 불리하다",
        {
    ...
    run(
        f"C. 펫 목록 + 알레르기 (user {user_id}, 펫 {pet_n}마리) - 1문장 둘 vs 2문장 셋",
        {
