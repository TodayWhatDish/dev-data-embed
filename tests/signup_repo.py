...
from app.core.db import execute, new_session
...
if __name__ == "__main__":
    with new_session() as db:
        ...
        assert find_user_by_email(db, email) is None, "이전 실행의 흔적이 안 지워졌다"

        user_id = create_user(db, email=email, name="테스트유저", password_hash=hash_password("pw12345"))
        ...
        execute(db, "DELETE FROM pet WHERE pet_id = ?", (pet_id,))
