...
from app.core.db import QueryError, execute, new_session
...
if __name__ == "__main__":
    with new_session() as db:
        ...
        categories = product_repo.get_product_categories(db)
        purposes = product_repo.get_feeding_purposes(db)
        ingredients = product_repo.get_ingredients(db)
        ...
        first = product_repo.find_page(db, 0, 5)
        second = product_repo.find_page(db, 1, 5)
        ...
        assert product_repo.find_by_id(db, -1) is None

        product_id = None
        try:
            # 4. 등록 - 안 넣은 컬럼은 DB DEFAULT 가 채운다. 그래서 등록 후 다시 SELECT 한다
            product_id = product_repo.insert(db, make_draft(categories[0]["product_category_id"]))
            created = product_repo.find_by_id(db, product_id)
            logger.info(
            ...
            assert product_repo.update_product(db, product_id, {"price_krw": 2000}) == 1
            assert product_repo.find_by_id(db, product_id)["price_krw"] == 2000

            # 7. 없는 id 는 UPDATE 가 터지지 않는다. 0행을 고치고 조용히 성공한다
            assert product_repo.update_product(db, -1, {"price_krw": 2000}) == 0
            ...
            if product_id is not None:
                assert product_repo.inactive_product(db, product_id) == 1
                assert product_repo.find_by_id(db, product_id)["is_active"] == 0
                assert product_repo.inactive_product(db, -1) == 0  # 없는 id 는 0행
