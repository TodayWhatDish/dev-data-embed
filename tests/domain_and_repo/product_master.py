...
def print_category_child(child, parent_category: str | None = None, tab_cnt = 0):
    ...
    """
    if parent_category:
        print("\t"*tab_cnt + f"{parent_category}")
...
if __name__ == '__main__':
    ...
    print(f"루트 {len(root)}개 / 전체 {len(hierarchy)}개")
    print("#"*20)

    print("id 로 한 건 조회 - 하위 카테고리까지")
    for category_id in list(hierarchy)[:3]:
        category = mgr.get_product_category(category_id)
        print(f"\t{category_id}: {category['name_ko']} <- 하위 {[c['name_ko'] for c in category['children']]}")
    print("#"*20)

    print("급여 목적 - {id: 급여목적 정보}")
    for purpose_id, purpose in mgr.get_feeding_purpose().items():
        print(f"\t{purpose_id}: {purpose['name_ko']}")
    assert len(mgr.get_feeding_purpose()) == len(get_feeding_purposes())
    print("#"*20)

    print("원료 - {id: 원료 정보}")
    ingredients = mgr.get_ingredient()
    print(f"\t{len(ingredients)}종: {[i['name_ko'] for i in list(ingredients.values())[:5]]} ...")
