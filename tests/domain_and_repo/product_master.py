...
from app.repositories.products import get_feeding_purposes, get_ingredients, get_product_categories
...
if __name__ == "__main__":
    with new_session() as db:
        ...
        mgr.set_product_category(get_product_categories(db))
        mgr.set_feeding_purpose(get_feeding_purposes(db))
        mgr.set_ingredient(get_ingredients(db))
        ...
        assert len(hierarchy) == len(get_product_categories(db))
        ...
        for category_id in list(hierarchy)[:3]:
            ...
            logger.info(
                f"\t{category_id}: {category['name_ko']} <- 하위 {[c['name_ko'] for c in category['children']]}"
            )
        logger.info("#" * 20)
        ...
        assert len(mgr.get_feeding_purpose()) == len(get_feeding_purposes(db))
        logger.info("#" * 20)
        ...
        assert len(ingredients) == len(get_ingredients(db))
        logger.info("#" * 20)

        logger.info("존재하지 않는 id")
        for name, value in (
            ("product_category", mgr.get_product_category(500)),
        ...
        for row in get_product_categories(db):
