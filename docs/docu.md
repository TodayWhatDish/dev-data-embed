...
### 마스터 데이터 (`animal_category` / `breed` / `allergen`)
...
- [ ] **품종 드롭다운을 `animal_category_id` 로 필터해서 채운다.** 개인 `pet` 에 고양이 품종을 연결하는 것을
      DB 가 막지 않는다(`pet_breed` 에 복합 FK 를 걸지 않기로 했다).
...
- [ ] **계층 재편(`parent_id` 이동)은 서비스 오픈 전에 끝낸다.** 계층을 바꾸면 이미 팬아웃된
      `pet_allergy` 행이 새 계층과 어긋난다. `단백질` 을 고른 사람은 그 시점 기준으로 펼쳐진 행을
      갖고 있어서, 나중에 `가금류` 가 중간에 생겨도 그 사람 행에는 없다.
- [ ] **원료를 등록할 때 알러지원 매핑을 같은 트랜잭션에서 넣는다.** `allergen_reviewed` 를 뺐으므로
      (2026-08-24) `ingredient_allergen` 0행은 곧 '알러지원 없음'으로 읽힌다. 매핑을 나중에
...
- [ ] **`purchase.age_month_at_purchase` 는 앱이 `petcalc.age_months()` 로 계산해 넣는다.**
      SQL 로 하면 `julianday` 차이를 30.44 로 나눠야 하는데 근사라 생일 근처에서 틀린다.
- [ ] **`pet.birth_date` 를 나중에 입력받으면 그 아이의 기존 구매를 백필한다.**
      안 하면 그 행들은 영영 NULL 이라 나이 기반 집계에서 빠진다.

### 체구

- [ ] **`pet.size` 를 고칠 때 그 이유를 구별해서 처리한다.** DB 는 못 구별한다.
      - **자랐다** (기본) → `pet.size` 만 UPDATE. 과거 `purchase.size_at_purchase` 는 그대로 둔다
      - **잘못 봤다가 정정** → 그 아이의 기존 `purchase.size_at_purchase` 도 같이 UPDATE
...
- [ ] **`local` 계정을 실제로 쓸 거면 두 가지를 먼저 정한다** — 비밀번호 해시를 둘 자리(`user` 에는 없다.
...
- [ ] **보관기간 경과분 파기도 `DELETE` 가 아니라 익명화 `UPDATE` 로 한다.**
      2026-08-24 부터 `pet.user_id` 와 `purchase.pet_id` 가 **RESTRICT** 라 `user`/`pet` 행 삭제를
      DB 가 거부한다. 실수로 후기 전체를 날리는 경로가 막혔다.
- [ ] **익명화 대상은 사람으로 되돌아가는 값뿐이다.** `user.email`/`name`/`phone`/`auth_uid`/`region`,
      그리고 `pet.name`. 품종·체구·체중·알러지는 개인정보가 아니고 세그먼트 추천의 근거다 — 지우지 않는다.
- [ ] **`review.body` 를 따로 본다.** 자유 텍스트라 스키마가 통제하지 못한다.
      가족관계·지역·연락처가 본문에 그냥 들어온다. 파기 검토가 실제로 필요한 자리는 여기다.
- [ ] **삭제권(GDPR 17조) 행사는 별개 경로다.** RESTRICT 때문에 `review` → `purchase` → `pet` → `user`
...
- [ ] `product.target_size_min`/`max` 는 `pet.size` 와 같은 `1~5` 코드를 쓴다.
- [ ] `animal_category_id` 를 갖는 모든 테이블은 `animal_category` 를 참조한다. 테이블마다 따로 매기지 않는다.

### 제품 분류 (2026-08-17 결정)

- [ ] **제품 하나에 분류 하나.** '사료 겸 간식'을 만들지 않는다 — 등록하는 사람이 간식이면 간식,
      사료면 사료로 무조건 하나를 고른다. 애매하면 "이것만 먹여도 되는가"로 가른다.
- [ ] **`product_category` 는 2단계까지만 쓴다** (대분류 → 소분류). 소분류의 소분류를 만들지 않는다.
...
- [ ] 축종을 늘리려면 `animal_category` INSERT 만으로는 부족하다 — `breed` 목록,
      `pet.size`/`body_type` 척도, 제품 급여 기준이 전부 그 축종용으로 딸려 와야 한다.
- [ ] **제품은 대상 축종을 반드시 등록한다.** `product_animal_category` 에 행이 0개면
