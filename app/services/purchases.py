...
def my_purchases(db: Session, user_id: int) -> list[dict]:
...
def buy(db: Session, user_id: int, product_id: int, quantity: int = 1) -> int:
    ...
    return purchases_repo.create_purchase(db, pets[0]["pet_id"], product_id, quantity, product["price_krw"])
...
def write_review(db: Session, user_id: int, purchase_id: int, rating: int, body: str) -> None:
    ...
    if not purchases_repo.is_owned_by(db, purchase_id, user_id):
        raise ValueError("본인 구매 내역이 아닙니다.")
    try:
        purchases_repo.create_review(db, purchase_id, rating, body)
