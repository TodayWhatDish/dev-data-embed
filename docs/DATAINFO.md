...
> **[2026-08-25] 새 CSV 는 `data/master/` 와 `data/seed/` 에 있습니다 — 설명은 [`../data/README.md`](../data/README.md).**
> 이 문서가 설명하는 `data/pet_*.csv` 4종은 **옛 4테이블 비정규화 스키마**의 것이고,
> 이제 `src/load_db.py` 만 읽습니다. 새 파이프라인(`gen_seed.py` → `load_csv.py`)은 읽지 않습니다.
> 값 분포(시/도 비중, 고객당 반려동물 수, 알러지 보유율)는 새 생성기가 여기서 실측해 옮겼으므로
> 아래 §1~3 은 그 재료로서 유효합니다.
>
> **[2026-08-13] 이 문서는 초기 더미 CSV 4종을 설명합니다. 현재 DB 스키마와 다릅니다.**
> 스키마를 `design/GOAL.md` 요구사항에서 역산해 14 테이블로 재설계했고, 확정본은 **`design/DESIGN.md`**입니다.
> `data/*.csv`는 이 스키마에 맞춰 다시 생성할 예정이라, 아래 내용은 그때까지의 참고 자료입니다.
>
> 주요 차이: ID가 `'C0001'` TEXT → 정수 대리키 / `age` → `birth_date` / 구매·후기 테이블 분리 /
> 알러지·원료·견종·급여목적이 마스터 테이블 참조로 분리.
>
> **§4는 아래 표기와 무관하게 이미 낡아 있습니다** — 옛 파일 기준(템플릿 3~4종, 4% 공백)으로 쓰였는데
> 현재 CSV 실측은 1,439건 / 고유 리뷰 1,422개 / 공백 0건 / 여러 문장입니다. §1~3은 CSV와 일치합니다.
