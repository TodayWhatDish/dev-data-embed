...
의존: pet_schema (pet), product_schema (product).
      그래서 execute_schema.MODULES 에서 그 둘보다 뒤에 온다.

후기는 구매에 매달린다(review.purchase_id 가 PK 이자 FK). 구매 없이 쓴 후기는 받지 않는다 —
...
CREATE TABLE purchase (
    ...
    pet_id                INTEGER NOT NULL
        REFERENCES pet(pet_id)         ON DELETE RESTRICT,
    product_id            INTEGER NOT NULL
        REFERENCES product(product_id) ON DELETE RESTRICT,
    quantity              INTEGER NOT NULL DEFAULT 1 CHECK (quantity > 0),

    -- 구매 시점의 값들. pet/product 를 조인하면 '지금' 값이 나온다. 근거는 md.
