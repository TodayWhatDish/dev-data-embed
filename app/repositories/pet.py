...
단일 테이블 CRUD 는 ORM(db.query/add)으로 간다. 조회가 조인이거나 GROUP_CONCAT 상관
...
def save_pet_survey(db: Session, pet_id: int, diet_note: str = None, skin_note: str = None) -> None:
...
def get_pet_survey(db: Session, pet_id: int) -> dict | None:
    ...
    return fetch_one(db, "SELECT diet_note, skin_note FROM pet_survey WHERE pet_id = %s", (pet_id,))


def get_breeds(db: Session):
    rows = db.query(Breed).all()
    return [as_dict(row) for row in rows]


def find_pets_by_user(db: Session, user_id: int) -> list[dict]:
    ...
    """
    return _fetch_pets(db, "p.user_id = %s AND p.inactive_at IS NULL", (user_id,), f"user_id={user_id}")


def find_pet(db: Session, pet_id: int) -> dict | None:
    ...
    """
    rows = _fetch_pets(db, "p.pet_id = %s", (pet_id,), f"pet_id={pet_id}")
    return rows[0] if rows else None


def _fetch_pets(db: Session, where: str, params: tuple, what: str) -> list[dict]:
...
def resolve_allergen_ids(db: Session, names: list[str]) -> list[int]:
    """알레르겐 이름 목록을 allergen_id 목록으로 바꾼다. DB에 없는 이름은 조용히 빠진다
    (오타로 필터가 통째로 안 걸리는 것보단, 아는 것만이라도 걸리는 게 낫다)."""
    if not names:
        return []
    marks = ", ".join("%s" for _ in names)
    rows = fetch_tuples(db, f"SELECT allergen_id FROM allergen WHERE name_ko IN ({marks})", tuple(names))
